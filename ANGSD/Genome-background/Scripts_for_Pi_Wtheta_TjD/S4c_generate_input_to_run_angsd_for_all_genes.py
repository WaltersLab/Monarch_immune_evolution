...
#### Run ANGSD for calculating popgen stats for all genes in the genome background.  ####
# Parse the annotation GFF file (OGS 2.0) to get positions for each gene. 
# Need to have angsd bam file list set up beforehand.
...
nonZ_list = [i.strip('\"') for i in nonZ_list]   # remove quote

## Parsing the GFF file to get info for all genes
		...
		for line in data.readlines():
			...
			if record[2] == "gene": # get the line that contains whole gene info
				...
				if ID in nonZ_list:				
					...
					os.system("S4d_ANGSD_calculations.sh " + args.population + " " + str(ID) + " " + str(region) + " " + args.sites) 
