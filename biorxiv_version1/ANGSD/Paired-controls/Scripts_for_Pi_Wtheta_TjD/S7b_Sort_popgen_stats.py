...
with open(args.gene_list, "r") as list: # open a list of samples
	for i in list:
		...
		# Tajima's D 
		if args.site == "full": 
			filename1 = str(gene)+"_outFold.thetas.idx.pestPG"
		else: 
		...
		with open(filename1, "r") as outfile:   # open each angsd final output file
			...
			TJ_stat = [line.split("\t")[8] for line in outfile][0]  # here is the Tajima's D value 
			output_d.write("%s\t%s\n" % (gene, TJ_stat))  # output
		
		# Watterson's theta and Pi 		
		if args.site == "full": 
			filename2 = str(gene)+"_outFold.thetas.idx.txt"
		else: 
		...
		output_t.write("%s\t%s\n" % (gene, np.nanmean(wtheta)))  # output the mean of the theta (use nanmean in case of nan present)
		output_p.write("%s\t%s\n" % (gene, np.nanmean(Pi)))  # output the mean of the pi (use nanmean in case of nan present)
